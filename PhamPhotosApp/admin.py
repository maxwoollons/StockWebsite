...
from .models import users, purchases, photos, payments, videos, videopurchases, exchange, savedphoto
...
class modphotos(admin.ModelAdmin):
    list_display = ['title','photo', 'approved', 'price','added']
...
class modphotoss(admin.ModelAdmin):
    list_display = ['title','video', 'approved', 'price','added']
    actions = [approve,]  # <-- Add the list action function here
